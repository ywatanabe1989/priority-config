...
# File: /home/ywatanabe/proj/priority-config/tests/custom/test_src_test_agreement.py
# ----------------------------------------
...
class AgreementChecker:
    ...
    def _create_test_file(self, src_file: Path, test_file: Path):
        ...
        # For src-layout packages, use the actual package import path
...
class TestSrcTestAgreement:
    ...
    def test_source_files_can_be_discovered(self):
        ...
        # Check that we found some expected files (universal check)
        source_file_names = {f.name for f in source_files}
        
        # Basic sanity check - should have at least __init__.py in any Python package
        assert "__init__.py" in source_file_names, "Expected __init__.py not found"
    ...
    def test_path_conversion_functions_work(self):
        ...
        # Use discovered source files instead of hardcoded paths
        source_files = self.checker.get_source_files()
        
        if source_files:
            sample_src = source_files[0]  # Use first discovered file
